...
| `v3` at 100k steps | same rung, budget raised | `configs/nytimes/v3_seed42_100k.yaml`, resumed from the row above | `runs/nytimes/v3_seed42_100k` (box: `/workspace/nytimes-v3/v3_seed42_100k`) | queued -- continuation from `checkpoint_step_30000.pt` via `scripts/nytimes_v3_seed42_100k_job.sh`, to see whether a longer budget settles the radius drift or only extends the post-selection decay |
