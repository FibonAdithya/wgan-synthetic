...
# set (3 checkpoints x 7 sets + real = 22 sets). The result is copied to
...
OUT=runs/nytimes/probes/trunk_scale_v2c.json
...
"$P" tools/probes/trunk_scale_probe.py \
  ...
  --output "$OUT"
mkdir -p "$KEEP" && cp "$OUT" "$KEEP"/ && sha256sum "$KEEP"/trunk_scale_v2c.json
