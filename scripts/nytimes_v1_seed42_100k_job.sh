#!/usr/bin/env bash
# One gpuq job: continue NYTimes v1 (seed 42) from its step-30,000 checkpoint
...
#     --lane gpu --timeout-s 14400 -- bash scripts/nytimes_v1_seed42_100k_job.sh
#
# runs/ is gitignored, so nothing here is declared as a --artifact; the run
# directory is copied to /workspace/nytimes-v1 at the end instead, the way the
...
RUN=runs/nytimes/v1_seed42_100k
RESUME=/workspace/nytimes-v1/v1_seed42/checkpoint_step_30000.pt
REAL=/workspace/data-cache/nytimes_250k.npy
CLEAN=/workspace/data-cache/nytimes_250k_l2_clean.npy
KEEP=/workspace/nytimes-v1/v1_seed42_100k
CANON="--ann-max-rows 20000 --ann-k 100 --ann-hub-k 10 --metric angular"

test -f "$REAL" && test -f "$CLEAN" && test -f "$RESUME"
"$P" -m src.train.train_wgan_gp --config configs/nytimes/v1_seed42_100k.yaml --resume "$RESUME"
# A resume restores best_score from the 30k run (0.0114 at step 20,000, under
# select_on: gate). If no later eval beats it, no new best_generator.pt is written and the selection
# is the 30k run's own file, so carry that one over rather than fail.
if [ ! -f "$RUN/best_generator.pt" ]; then
  cp /workspace/nytimes-v1/v1_seed42/best_generator.pt "$RUN/best_generator.pt"
...
"$P" -m src.eval.eda_report --real-path "$CLEAN" \
  ...
  --synthetic-path "v1_step100000=$RUN/synthetic_50k_step100000.npy" \
