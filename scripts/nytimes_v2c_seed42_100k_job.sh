#!/usr/bin/env bash
# One gpuq job: continue NYTimes v2c (seed 42) from its step-30,000 checkpoint
# to 100,000 steps, sample the final and selected checkpoints, and measure
# them against the cleaned real corpus.
#
#   gpuq submit --project wgan-synthetic --commit <sha> --branch nytimes-v2c-100k \
#     --lane gpu --timeout-s 18000 -- bash scripts/nytimes_v2c_seed42_100k_job.sh
#
# The timeout is sized from the 30k run: 67 minutes for 30,000 steps plus
# sampling and the report, so 70,000 more steps is about 160 minutes; 18,000 s
# leaves the same margin the v1 continuation had.
#
# runs/ is gitignored, so nothing here is declared as a --artifact; the run
# directory is copied to /workspace/nytimes-v2 at the end instead, the way the
# v1 continuation did. Box-specific by construction, like the config it runs.
set -euo pipefail
P=${WGAN_PYTHON:-/opt/venvs/wgan-synthetic/bin/python}
RUN=runs/nytimes/v2c_seed42_100k
RESUME=/workspace/nytimes-v2/v2c_seed42/checkpoint_step_30000.pt
REAL=/workspace/data-cache/nytimes_250k.npy
CLEAN=/workspace/data-cache/nytimes_250k_l2_clean.npy
KEEP=/workspace/nytimes-v2/v2c_seed42_100k
CANON="--ann-max-rows 20000 --ann-k 100 --ann-hub-k 10 --metric angular"

test -f "$REAL" && test -f "$CLEAN" && test -f "$RESUME"
"$P" -m src.train.train_wgan_gp --config configs/nytimes/v2c_seed42_100k.yaml --resume "$RESUME"
# A resume restores best_score from the 30k run (0.0190 at step 4,000, under
...
if [ ! -f "$RUN/best_generator.pt" ]; then
  cp /workspace/nytimes-v2/v2c_seed42/best_generator.pt "$RUN/best_generator.pt"
...
"$P" -m src.eval.eda_report --real-path "$CLEAN" \
  ...
  --synthetic-path "v2c_step100000=$RUN/synthetic_50k_step100000.npy" \
