...
def test_distance_is_finite_under_half_precision_inputs():
    ...
    Under AMP the generator step runs inside autocast, so `fake` arrives as
    fp16. eigvalsh on an fp16 covariance is badly conditioned where it is
    implemented at all, so `_normalized_spectrum` casts to float32 first.
    Without that cast this raises or returns NaN.
    """
    real = _anisotropic(256, 32, 0.05, seed=0).half()
    fake = _anisotropic(256, 32, 0.50, seed=1).half().requires_grad_(True)
    out = spectrum_distance(real, fake)
