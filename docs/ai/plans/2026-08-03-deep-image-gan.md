...
> **Archived.** Written before the multi-dataset refactor (PR #7) landed, so
> the paths below are the ones that existed at the time: `src/deep/*`,
> `configs/deep_gan_v*.yaml`, `src/data/sift1m_dataset.py`. The shipped code
> was reconciled onto the current layout when this branch was merged — see
> `docs/datasets/deep.md` for where each piece actually lives.
